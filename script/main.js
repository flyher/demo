...
var strDelConfirm =
  "This will DELETE your game data and saved games stored in browser cache. Type 'YES' to continue.";
...
if (userLang === 'zh-CN' || userLang.startsWith('zh-Hans')) {
  ...
  strLoading = '正在加载';
  strDelConfirm =
    '此操作将删除您浏览器缓存中保存的数据文件及存档。请输入 "YES" 继续：';
  ...
  strLoading = '正在加載';
  strDelConfirm =
    '此操作將刪除您瀏覽器緩存中保存的遊戲資料檔及記錄。請輸入 "YES" 繼續：';
}
...
var Module = {
  ...
  setStatus: function (text) {
    ...
    if (m) {
      text = m[1];
      progressElement.setAttribute('aria-valuenow', parseInt(m[2]) * 100);
      progressElement.setAttribute('valuemax', parseInt(m[4]) * 100);
      progressElement.style.width = '${parseInt(m[2]) * 100}%';
      // spinnerElement.hidden = false;
    } else {
      progressElement.setAttribute('aria-valuenow', 0);
      progressElement.setAttribute('valuemax', 100);
      ...
      if (zipEntry.dir) {
        ...
        zip.sync(function () {
          zipEntry.async('uint8array').then(function (arr) {
            FS.writeFile('/data/' + relativePath.toLowerCase(), arr, {
              encoding: 'binary'
            });
          });
        });
...
async function runGame() {
  mainFunc = Module.cwrap('EMSCRIPTEN_main', 'number', ['number', 'number'], {
    async: true
  });
  mainFunc(0, 0);
}
